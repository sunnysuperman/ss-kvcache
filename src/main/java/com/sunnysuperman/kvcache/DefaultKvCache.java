...
public class DefaultKvCache<T, K> implements KvCache<T, K> {
	...
	protected RepositoryProvider<T, K> repository;
	protected Converter<T> converter;
	protected KvCacheSaveFilter<T, K> saveFilter;

	public DefaultKvCache(KvCacheExecutor executor, KvCachePolicy policy, RepositoryProvider<T, K> repository,
			Converter<T> converter, KvCacheSaveFilter<T, K> saveFilter) {
	...
	public RepositoryProvider<T, K> getRepository() {
		return repository;
	}
	...
	public KvCacheSaveFilter<T, K> getSaveFilter() {
		return saveFilter;
	}
	...
	public Map<K, T> findByKeys(Collection<K> keys, boolean cacheOnly) throws KvCacheException {
		...
		if (queryKeys.isEmpty()) {
			LOG.warn("queryKeys is empty, should not happen!!!");
			return map;
		}
		// 从存储层中查找
		Map<K, T> freshMap;
